...
select count(*) completed_trips from trips_details4
where end_ride=1;
...
select p.method from payment p
join(
select faremethod,max(fare)as highest_fare from trips
group by faremethod
...
-- highest amount paid by a mode in a day
select p.method,b.highest_total_fare from payment p
join(
select faremethod,sum(fare)as highest_total_fare from trips
group by faremethod
order by highest_total_fare desc
limit 1) b
on p.id=b.faremethod;

-- which two location has most trips
select * from
(select *,dense_rank() over(order by cnt desc) rnk from(
select loc_from,loc_to,count(distinct tripid) as cnt
from trips
...
select *
from(
select *,dense_rank() over( order by total1 desc) as rk
from
(select driverid,sum(fare) as total1
from trips
group by driverid
)b
)c
...
from
(select duration,count(tripid) as cnt
from trips
group by duration
)b
)c
...
-- which driver,customer had more 
select *
from(
...
select *
from(
select *,dense_rank() over(partition by duration order by cnt desc) as rnk
from(
...
-- which duration area got highest trips
select *
from(
select *,dense_rank() over(partition by loc_from order by cnt desc) as rnk
from(
select duration,loc_from,count(distinct tripid) as cnt
from trips
group by duration,loc_from
...
select * from(
select *,rank() over(order by cnt desc) rnk from(
select duration,count(tripid) cnt
from trips
group by duration)b
)c
where rnk=1;
